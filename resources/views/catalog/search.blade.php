        ...
        <div class='card' style='overflow:visible;width:100%;height:100%;'>
            ...
            @if($headers[$j]->type=='header')
                        ...
                        @elseif($headers[$j]->type=='text')
                            ...
                            class='inv'
                            @endif
                            style='{{$headers[$j]->styles}}' id='{{ $headers[$j]->name }}_{{ $headers[$j]->cat_field_id}}' ><b>{{ $headers[$j]->name }}</b>:&nbsp;{{ $entries[$i+$j]->value}}</span>
                        @elseif($headers[$j]->type=='desc')
                            <span 
                            @if($headers[$j]->visibility==0)
                           class='inv'
                            @endif
                             id='{{ $headers[$j]->name }}_{{ $headers[$j]->cat_field_id}}' style='{{ $headers[$j]->styles }}'><b>{{ $headers[$j]->name }}</b>:&nbsp;{{$entries[$i+$j]->value}}</span>
                        @elseif($headers[$j]->type=='link')
                            <a 
                            @if($headers[$j]->visibility==0)
                            class='inv'
                            @endif
                             style='{{$headers[$j]->styles}}' target='_blank' id='{{ $headers[$j]->name }}_{{ $headers[$j]->cat_field_id}}' href='{{$entries[$i+$j]->value}}' ><b>{{ $headers[$j]->name }}</b>:&nbsp;{{ __('Click here.') }}</i></a>
                        @endif

...
<div class='row'>
    <div class='col-md-10' id='product_zone'>
        <ul style='display: inherit;width:100%;height:100%;'>
            <li class='threecols'>
                <div class='card' id='prod_card'>
                    @for($i=0;$i<count($headers);$i++)
                        ...
                        @if($headers[$i]->type=='header')
                            ...
                            @endif
                            '
                            style='border:0px!important' align='center' id='{{ str_replace(array(' ','?','#',':',';',',','=','.'),'',$headers[$i]->name) }}_{{ $headers[$i]->cat_field_id}}'><b>{{ $headers[$i]->name }}</b>:&nbsp;{{$examp[$i]->value}}&nbsp;<i class='fa fa-cog' onclick='config_header(this)'></i></div>
                        ...
                        @elseif($headers[$i]->type=='desc')
                            ...
                            inv
                            @endif
                            ' id='{{ str_replace(array(' ','?','#',':',';',',','=','.'),'',$headers[$i]->name) }}_{{ $headers[$i]->cat_field_id}}' style='width:80%;height:30%;'><b>{{ $headers[$i]->name }}</b>:&nbsp;{{$examp[$i]->value}}&nbsp;<i class='fa fa-cog' onclick='config_header(this)'></i></span>
                          @elseif($headers[$i]->type=='text')
                            <span class='drag_resize
                            @if($headers[$i]->visibility==0)
                            inv
                            @endif
                            ' id='{{ str_replace(array(' ','?','#',':',';',',','=','.'),'',$headers[$i]->name) }}_{{ $headers[$i]->cat_field_id}}' style='width:80%;height:25%;'><b>{{ $headers[$i]->name }}</b>:&nbsp;{{$examp[$i]->value}}&nbsp;<i class='fa fa-cog' onclick='config_header(this)'></i></span>
                        @elseif($headers[$i]->type=='link')
                            <div style='width:60%;height:15%;' id='link_wrapper' class='drag_resize

                            @if($headers[$i]->visibility==0)
                            inv
                            @endif
                            ' >
                            <a target='_blank' id='{{ str_replace(array(' ','?','#',':',';',',','=','.'),'',$headers[$i]->name) }}_{{ $headers[$i]->cat_field_id}}' href='{{$examp[$i]->value}}' ><b>{{ $headers[$i]->name }}</b>:&nbsp;{{ __('Click here.') }}</a><i class='fa fa-cog' onclick='config_header(this)'></i></div>
                        @endif
                    @endfor
                </div>
            </li>
...
<br><br>
<br><br>
<script>$("select").selectpicker();layouterevents();</script>